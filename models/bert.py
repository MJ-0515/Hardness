...
class BertTextEncoder(nn.Module):
    def __init__(self, use_finetune=False, transformers='bert', pretrained='bert-base-uncased'):
        ...
        tokenizer_class = TRANSFORMERS_MAP[transformers][1] # BertTokenizer
        model_class = TRANSFORMERS_MAP[transformers][0]  #BertModel
    ...
    def forward(self, text):
        ...
        if self.use_finetune:
            ...
            # last_hidden_states = self.model(input_ids=input_ids, attention_mask=input_mask).last_hidden_state
        else:
            with torch.no_grad():
                last_hidden_states = self.model(input_ids=input_ids,
                                                attention_mask=input_mask,
