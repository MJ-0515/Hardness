...
def get_best_results(results, best_results, epoch, model, optimizer, ckpt_root, seed, type, save_best_model):
    if epoch == 1:
        ...
        for key, value in results.items():
            if (key == 'Has0_acc_2') and (value > best_results[key]):
                ...
                if save_best_model:
                    key_eval = 'Has0_acc_2'
                    ckpt_path = os.path.join(ckpt_root, f'best_{type}_{key_eval}_{seed}.pth')
                    print("-----------------save new Has0_Acc_2/F1 ckpt--------------------------------")
                ...
                if save_best_model:
                    key_eval = 'Non0_acc_2'
                    ckpt_path = os.path.join(ckpt_root, f'best_{type}_{key_eval}_{seed}.pth')
                    print("-----------------save new None0_Acc_2/F1 ckpt--------------------------------")
                    save_model(ckpt_path, epoch, model, optimizer)
                ...
                if save_best_model:
                    key_eval = 'MAE'
                    ckpt_path = os.path.join(ckpt_root, f'best_{type}_{key_eval}_{seed}.pth')
                    print("-----------------save new MAE ckpt--------------------------------")
                    save_model(ckpt_path, epoch, model, optimizer)

            elif key == 'Mult_acc_2' and (value > best_results[key]):  #for SIMS
                best_results[key] = value
                ...
                if value > best_results[key]:
                    ...
                    if save_best_model:
                      key_eval = key
                      ckpt_path = os.path.join(ckpt_root, f'best_{type}_{key_eval}_{seed}.pth')
                      print("-----------------save new acc_5/7/corr ckpt--------------------------")
                      save_model(ckpt_path, epoch, model, optimizer)
